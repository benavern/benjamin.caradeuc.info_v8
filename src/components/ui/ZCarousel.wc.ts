...
export class ZCarousel extends LitElement {
    ...
    private get _perMove() {
        return clamp(this.perMove, { min: 1, max: this._perPage });
    }
                ...
                ${map(
                    ...
                    (_slideEl: HTMLElement, index: number) => html`
                        <button
                            ...
                            aria-hidden="${(index + 1) !== this._currentPage}"
    ...
    static styles = css`
        ...
        * {
        ...
        .carousel__content {
            ...
            grid-auto-columns: calc((100% / var(--_z-carousel-per-page)) - (var(--_z-carousel-gap, 0px) * (var(--_z-carousel-per-page) - 1) / var(--_z-carousel-per-page)));
            grid-auto-rows: 100%;
